...
| 版本 | 日期 | 內容 |
|---|---|---|
| v2.3.2 | 2026-07-06 | 結果視窗的顯示用 AutoSTF 由三色版連動（linked）改為各色版獨立（unlinked，與 PixInsight Ctrl+A 預設一致）：連動拉伸在三色版背景稍有不平衡時會讓整個畫面染色、對比被壓掉，容易被誤判為 DBE 失敗（僅影響螢幕顯示，不影響像素資料） |
| v2.3.1 | 2026-07-06 | 輸出視窗保留原始中繼資料：複製視窗與背景模型現在會帶上來源影像的 FITS keywords（拍攝日期時間、經緯度、赤經赤緯、焦距等）、XISF view properties 與天體測量解，修正 DBE 後接 ImageSolver 時原始資訊全部遺失的問題 |
